...
def subsaltshaker(disk):
    ...
    current_digit = 0
    count = 1
    while (current_digit+count < len(str(disk))):
        last = str(disk)[current_digit+count]
        if str(disk)[current_digit]==last:
            count += 1
        else:
            count = 1
            current_digit+=1
        if count == int(str(disk)[current_digit]) or int(str(disk)[current_digit-1]) == 1:
            return True
        disk = disk
    return False or disk==1
