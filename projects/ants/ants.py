...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if isinstance(self,ContainerAnt) and not isinstance(place.ant,ContainerAnt) and self.can_contain(place.ant):
                self.contain_ant(place.ant)
                place.ant=self
            elif isinstance(place.ant,ContainerAnt) and not isinstance(self,ContainerAnt) and place.ant.can_contain(self):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place_times=0
        while len(new_place.bees)==0 or place_times<self.min_range:
            new_place=new_place.entrance
            place_times+=1
            if(new_place==beehive):
                return None
        if place_times>self.max_range:
            return None
        return rANTdom_else_none(new_place.bees)
        # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if place.bees != None:
            ...
            index=0
            while(index<leng):
                Insect.reduce_armor(place.bees[index],amount)
                nlen=len(place.bees)
                if nlen<leng:
                    index=index
                    leng=nlen
            ...
            if place.ant==None:
                ...
                index = 0
                while (index < leng):
                    Insect.reduce_armor(place.bees[index], damage)
                    nlen = len(place.bees)
                    if nlen < leng:
                        index = index
                        leng = nlen
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if place.bees != None:
            ...
            index=0
            while(index<leng):
                Insect.reduce_armor(place.bees[index],self.damage)
                nlen=len(place.bees)
                if nlen<leng:
                    index=index
                    leng=nlen
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.is_False = False
        QueenAnt.queen_instantiated+=1
        if QueenAnt.queen_instantiated==2:
            self.is_False=True
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.is_False is True:
            self.reduce_armor(self.armor)
            QueenAnt.queen_instantiated-=1
        else:
            tmp_place=self.place.exit
            while tmp_place is not None:
                if tmp_place.ant is not None and tmp_place.ant.is_doubled is False:
                    tmp_place.ant.damage*=2
                    tmp_place.ant.is_doubled=True
                if isinstance(tmp_place.ant, ContainerAnt) and tmp_place.ant.contained_ant is not None and tmp_place.ant.contained_ant.is_doubled is False:
                    tmp_place.ant.contained_ant.damage *= 2
                    tmp_place.ant.contained_ant.is_doubled = True
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        place=self.place
        is_false_queen=self.is_False
        ScubaThrower.reduce_armor(self,amount)
        if place.ant is None and is_false_queen is False:
            bees_win()
        # END Problem 13
    def remove_from(self, place):
        if self.is_False is False and self.armor!=0:
            self.place=place
        else:
...
class Bee(Insect):
    ...
    def __init__(self,armor,place=None):
        ...
        self.direction=True #True means Right
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        distance=0
        while tmp_place is not beehive:
            if tmp_place.ant is not None:
                if isinstance(tmp_place.ant,ContainerAnt):
                    insects[tmp_place.ant]=distance
                    if tmp_place.ant.contained_ant is not self:
                        insects[tmp_place.tmp_place.ant.contain_ant]=distance
                else:
                    if tmp_place.ant is not self:
                        insects[tmp_place.ant]=distance
            if tmp_place.bees is not None:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        total_damage=2-0.05*self.insects_shot
        damage_result=total_damage-0.2*distance
        if damage_result<0:
            return 0
        else:
