...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        "*** YOUR CODE HERE ***"
        if args is not nil:
            python_args=python_args+[args.first]
            new_args=args.rest
        else:
            new_args=nil
        while isinstance(new_args,Pair):
            python_args=python_args+[new_args.first]
            new_args=new_args.rest
        if new_args is not nil:
            python_args=python_args+[new_args]
        if self.use_env is True:
            python_args=python_args+[env]
        try:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        lambda_expressions=do_lambda_form(Pair(target.rest,expressions.rest),env)
        env.define(target.first,lambda_expressions)
        return target.first
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    eval_exp=scheme_eval(expressions.first,env)
    if expressions.rest is nil and is_true_primitive(eval_exp):
        return eval_exp
    else:
        if is_false_primitive(eval_exp):
            return False
        else:
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        ...
        if is_true_primitive(eval_expr):
            return eval_expr
        else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        eval_value=scheme_eval(bindings.first.rest.first,env)
        values=Pair(eval_value,values)
...
class MuProcedure(Procedure):
    """A procedure defined by a mu expression, which has dynamic scope.
     _________________
    < Scheme is cool! Do you play Computer Science Genshin Impart?>
     -----------------
            \   ^__^
             \  (oo)\_______
                (__)\       )\/\
