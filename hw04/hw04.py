    ...
    if is_planet(m):
        ...
        if is_mobile(end(right(m))):
            if not balanced(end(right(m))):
                return False
        if left_length*total_weight(end(left(m)))==right_length*total_weight(end(right(m))):
            return True
        else:
            return False
...
def preorder(t):
    ...
    "*** YOUR CODE HERE ***"
    result=[]
    def preorder_with_list(t,result):
        if is_leaf(t):
            result.append(label(t))
        else:
            result.append(label(t))
            for branch in branches(t):
                preorder_with_list(branch,result)
        return result
    return preorder_with_list(t,result)
...
def quadratic(x, a, b, c):
    ...
    if a==0:
        ...
        delta = -b / (2 * a)
        if delta>upper_bound(x):
            return interval(compute(upper_bound(x)),compute(lower_bound(x)))
        elif delta<lower_bound(x):
            return interval(compute(lower_bound(x)),compute(upper_bound(x)))
        else:
            return interval(compute(delta),max(compute(upper_bound(x)),compute(lower_bound(x))))
    else:
        delta=-b/(2*a)
        if delta>upper_bound(x):
            return interval(compute(lower_bound(x)), compute(upper_bound(x)))
            ...
            return interval(min(compute(upper_bound(x)),compute(lower_bound(x))),compute(delta))
def par1(r1, r2):
...
def check_par():
    ...
    True
    """
    r1 = interval(1.1, 1) # Replace this line!
    r2 = interval(1.5, 1) # Replace this line!
    return r1, r2
