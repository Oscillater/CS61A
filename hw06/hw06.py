...
class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self,given_type,given_price):
...
def is_bst(t):
    ...
    if len(t.branches)>2:
        ...
        if((min<t.label and max<t.label) or (min>t.label and max>t.label)):
        ...
        if (t.branches[0].label<=t.label and t.branches[1].label>=t.label) and (max<=t.label) and (min>=t.label):
...
def store_digits(n):
    ...
    if n<10:
        ...
        while tmp>10:
            tmp=tmp//10
            times=times*10
        return Link(tmp,store_digits(n%times))
...
def deep_map(f, link):
    ...
    if isinstance(link.first,Link) and link.first!=Link.empty:
        nlink.first=deep_map(f,link.first)
    else:
        if link.first!=Link.empty:
            nlink.first=f(link.first)
    if isinstance(link.rest,Link) and link.first!=Link.empty:
        nlink.rest=deep_map(f,link.rest)
    else:
        if link.rest!=Link.empty:
            nlink.rest=f(link.rest)
    return nlink
