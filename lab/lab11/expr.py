...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        operator_evaluated=self.operator.eval(env)
        operands_evaluated=[]
        for operand in self.operands:
            operands_evaluated=operands_evaluated+[operand.eval(env)]
        try:
            result=operator_evaluated.apply(operands_evaluated)
            if result is not None:
                return result
            else:
                return
        except OverflowError as e:
            print('Oops! That is an overflow error!')
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        copy_environment=self.parent.copy()
        copy_environment.update(zip(self.parameters,arguments))
        return self.body.eval(copy_environment)
