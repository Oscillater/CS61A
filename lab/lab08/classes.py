...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for elem in player.deck.cards:
            elem.defense=other_card.defense+elem.defense
            elem.attack=other_card.attack+elem.attack
        copy=opponent.deck.cards[:]
        index=0
        for elem in copy:
            if elem.defense==other_card.defense and elem.attack==other_card.attack:
                opponent.deck.cards=opponent.deck.cards[:index]+opponent.deck.cards[index+1:]
                index=-1
            index+=1
