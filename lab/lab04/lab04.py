...
def reverse_recursive(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    def reverse_lst(oldlst,newlst):
        num=len(oldlst)-1
        if num>-1:
            newlst.append(oldlst.pop(num))
            return reverse_lst(oldlst,newlst)
        else:
            return newlst
    nl=[]
    return reverse_lst(lst,nl)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    if distance(city_a,location)>distance(city_b,location):
...
def add_chars(w1, w2):
    ...
    "*** YOUR CODE HERE ***"
    new=''
    def add_characters(w1,w2,newchar):
        if len(w1)>0:
            char=w1[0]
            num=w2.find(char)
            newchar=newchar+w2[:num]
            w1=w1[1:]
            w2=w2[num+1:]
            return add_characters(w1,w2,newchar)
        else:
            newchar=newchar+w2
            return newchar
    return add_characters(w1,w2,new)