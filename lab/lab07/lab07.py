...
def subseqs(s):
    ...
    if s==[]:
        ...
        result=subseqs(s[1:])+insert_into_all(s[0],subseqs(s[1:]))
        return result
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return insert_into_all(prev, subseq_helper(s[1:],s[0])) + subseq_helper(s[1:],s[0])
...
def make_glookup(class_assignments):
    ...
    total=0
    current=0
    def look_up(key,score):
        nonlocal total
        total=class_assignments[key]+total
        nonlocal current
