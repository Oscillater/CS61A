...
def in_order_traversal(t):
    ...
    if t.is_leaf():
        ...
        for order in in_order_traversal(t.branches[0]):
            yield order
        yield t.label
        for order in in_order_traversal(t.branches[1]):
            yield order
...
def interleaved_sum(n, odd_term, even_term):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper_sum(time, is_odd):
        if is_odd is True:
            if time==n:
                return odd_term(time)
            else:
                return helper_sum(time+1,False)+odd_term(time)
        else:
            if time==n:
                return even_term(time)
            else:
                return helper_sum(time+1,True)+even_term(time)
    return helper_sum(1,True)
...
def mutate_reverse(link):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(link,result):
        if link is not Link.empty:
            return helper(link.rest,Link(link.first,result))
        else:
            return result
    result=helper(link.rest,Link(link.first))
    link.first=result.first
    link.rest=result.rest
